...
import ModalComment from "../modal-comment/comment";
import Pagination from "../pagination/pagination";
import { parse } from "path";
import { addItemToCart, getLocalStorageWithTime } from "@/app/lib/helps";
import RecommendProduct from "../rcproduct/product";
...
const Product = ({ product }) => {
  ...
  let cart = getLocalStorageWithTime('cart')
  ...
  const addToCart = async () => {
    ...
    const httpRes = await addItemToCart(cart.id, { productId: product.id, quantity: 1 })
    if (httpRes.status === 401) {
      alert('Vui lòng đăng nhập lại để sử dụng chức năng này!')
      window.location.href = '/login'
      return
    }
    updateContextCart(1)
    ...
    <>
      ...
      <div className="container mx-auto">
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-1">
            ...
            <div className="grid grid-cols-2 mt-5">
              ...
              <button
                type="button"
                onClick={addToCart}
                className="flex justify-center gap-4 items-center text-white bg-orange-400 hover:bg-orange-500 focus:outline-none focus:ring-4 focus:ring-orange-300 font-semibold rounded-full text-lg px-5 py-2.5 text-center me-2 mb-2"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 576 512"
          ...
          <div className="col-span-1 bg-white mt-2 rounded-lg">
            <div
              className="p-2 flex flex-col items-between justify-between"
              style={{ height: "100%" }}
            >
              <div className="p-3">
                <div className="flex gap-1 items-center">
                  <div className="me-2 flex text-orange-500 gap-1 items-center">
                ...
                <h3 className="font-bold text-3xl line-clamp-3">
                  {product.name}
                </h3>
                {/* <div className="mt-4">
                  ...
                  new Date() < new Date(product.endSale) ? (
                    <>
                        ...
                        {product.type_discount === "percent"
                          ? formatMoney(
                              product.regularPrice -
                                (product.regularPrice * product.discount) / 100
                            )
                          : formatMoney(
                      ...
                      <div className="flex gap-1">
                        <p className="me-2 mb-0 line-through text-gray-500 text-lg">
                    ...
                    <>
                      <h3 className="text-red-600 mb-0 font-bold text-2xl">
                        {formatMoney(product.regularPrice)}
                      </h3>
                    </>
                  )}
              ...
              {/* <form className="p-3 max-w-xs flex gap-5 items-center">
                <label
                ...
                <div className="relative flex items-center max-w-[8rem]">
                  <button
                    ...
                    className="bg-blue-500 border border-gray-300 rounded-s-lg p-3 h-8 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none"
                  >
                    <svg
                  ...
                  <input
                    ...
                    className="bg-white border-x-0 border-gray-300 h-8 text-center text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full py-2.5"
                    placeholder="999"
                    value={1}
                    readOnly
                    required
                  ...
                  <button
                    ...
                    className="bg-blue-500 border border-gray-300 rounded-e-lg p-3 h-8 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none"
                  >
                    <svg
                      className="w-3 h-3 text-gray-900 dark:text-white"
                      aria-hidden="true"
          ...
          <div className="col-span-2 bg-white mt-7 rounded-lg">
            <div className="mt-7 px-7">
              <h2 className="font-bold text-2xl mb-5">Thông tin sản phẩm</h2>
              <div dangerouslySetInnerHTML={{ __html: product.description }} />
            </div>
            <div className="mt-7 p-7">
              <h2 className="font-bold text-2xl mb-5">Đánh giá sản phẩm</h2>
              <div className="col-span-1">
                <div className="flex justify-between">
                  <div className="flex items-center flex-wrap">
                    <h2
                      className="font-bold text-2xl me-3 flex gap-2 items-center"
                      style={{ color: "#e5780b" }}
                    >
                  ...
                  {pagination &&
                    product.product_reviews.map((review, index) => {
                      ...
                      if (index >= start && index < end) {
                        return (
                          <li key={index}>
                            <div className="cmt-name flex gap-5">
                              ...
                              <div className="flex gap-1 items-center">
                                ...
                                <span className="text-sm text-green-700">
                                  Đã mua hàng tại TechStore
                                </span>
                              </div>
                            </div>

                            <div
                              className="cmt-rating flex my-1"
                              style={{ color: "#e5780b" }}
                            >
                              {[1, 2, 3, 4, 5].map((key, index) => {
                                return (
                            ...
                            <div className="cmt-content text-sm my-2">
                              {review.content}
                            </div>
          ...
          <div className="col-span-2 bg-white mt-7 rounded-lg mb-10">
            <RecommendProduct productlist={product.similarProducts}/>
          </div>
        </div>
      </div>
    </>
